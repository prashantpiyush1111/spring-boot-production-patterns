...
```mermaid
flowchart TD
    A[Incoming Requests] --> B[Service Layer]
    B --> C[OrderService Thread Pool<br/>2 Threads]
    B --> D[PaymentService Thread Pool<br/>2 Threads]
    C --> E[Order Processing]
    D --> F[Payment Processing]
    C -. Slow or overloaded .-> G[Other pools remain isolated]
```
